...
for file in `ls $dir`; do
    ...
    cp query/$file-A2A-default.query A2A.query
    cp faceweight/$file-face_weight-default.query face_weight.query 

    bash parallel_run.sh $dir $file $gridnum 0.2 $spnum $qnum FixedS 10 $flag
    # bash parallel_run.sh $dir $file $gridnum $eps 5 $qnum UnfixedS 10 $flag
    # bash parallel_run.sh $dir $file $gridnum $eps 5 $qnum KAlgo 5 $flag
    # bash parallel_run.sh $dir $file $gridnum 0.2 $spnum $qnum SE 1 $flag
    bash parallel_run.sh $dir $file $gridnum 0.2 $spnum $qnum EAR 1 $flag
