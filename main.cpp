...
void getLambdaGamma(string &file_name, int point_num) {
    ...
    ifstream fin(file_name);
    fin >> surface_mesh;
    double S_min = 1e100, x_min = 1e100, x_max = -1.0, y_min = 1e100, y_max = -1.0;
    for (auto fd: surface_mesh.faces()){
        for (auto ed: surface_mesh.halfedges_around_face(surface_mesh.halfedge(fd))){
            ...
            double S_len = sqrt((s.x() - t.x()) * (s.x() - t.x()) + (s.y() - t.y()) * (s.y() - t.y()));

            if (Base::doubleCmp(S_len - S_min) < 0) S_min = S_len;
        }
    }
    for (auto vd: surface_mesh.vertices()){
        auto p = surface_mesh.points()[vd];
        if (Base::doubleCmp(p.x() - x_min) < 0) x_min = p.x();
        if (Base::doubleCmp(p.x() - x_max) > 0) x_max = p.x();
        if (Base::doubleCmp(p.y() - y_min) < 0) y_min = p.y();
        if (Base::doubleCmp(p.y() - y_max) > 0) y_max = p.y();
    }
    double S_max = max((x_max - x_min), (y_max - y_min));
